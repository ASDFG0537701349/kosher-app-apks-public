...
object AppPresentationUtils {
     ...
     * Returns null if no secondary action should be shown.
     */
    fun secondaryActionLabelRes(app: StoreApp, mode: AppCardMode): Int? {
        // Detail screen usually uses STORE mode.
        return when (mode) {
            AppCardMode.STORE -> {
                when (app.installStatus) {
                    ...
                    // Explicitly return null for all other statuses including INSTALLED
                    else -> null
                }
            }

            AppCardMode.MANAGEMENT -> {
                // In management screen, we show 'Uninstall' if installed
                if (app.isInstalled) R.string.action_uninstall else null
            }
        }
    }
