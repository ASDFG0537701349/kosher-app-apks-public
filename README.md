...
1. **הרצת סקריפט לעדכון תיאורים** (פעם ראשונה או לפי צורך):
   ```bash
   python tools/update_descriptions.py
   ```
   הסקריפט יוצר `apps_updated.json` with תיאורים מעודכנים מ-Google Play.

2. **העלאת הקבצים ל-GitHub**:
   - `apps_updated.json` - הקובץ שהאפליקציה קוראת ממנו
   - `apps.json` - קובץ המקור (לא חובה, אבל מומלץ לשמור כגיבוי)
   - אייקונים בתיקיית `icons/`

3. **האפליקציה תקרא מ-`apps_updated.json`** - זה הקובץ שהיא משתמשת בו כמקור המידע הראשי.
